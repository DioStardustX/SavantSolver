  ...
  if (!validNextStates.includes(newStatus)) {
    ...
    if (currentStatus === "Cerrado") {
      throw AppError.badRequest(
        `No se puede cambiar el estado de un ticket cerrado. Estado actual: ${currentStatus}`
      );
    }
 ...
 */
export const validateTechnicianRequired = (
  newStatus: TicketStatus,
  assignedTechnicianId: number | null | undefined
): void => {
  // Estados que requieren técnico asignado
  const statesRequiringTechnician: TicketStatus[] = [
    "Asignado",
    "En_Proceso",
    "Resuelto",
    "Cerrado",
  ...
  if (
    statesRequiringTechnician.includes(newStatus) &&
    !assignedTechnicianId
  ) {
    throw AppError.badRequest(
      `El estado "${newStatus}" requiere que el ticket tenga un técnico asignado. ` +
 ...
 */
export const validateCommentRequired = (
  comment: string | undefined
): void => {
  if (!comment || comment.trim().length === 0) {
    throw AppError.badRequest(
      "Se requiere un comentario obligatorio al cambiar el estado del ticket. " +
 ...
 */
export const validateImagesRequired = (
  images: string[] | undefined
): void => {
  if (!images || !Array.isArray(images)) {
    throw AppError.badRequest(
      "Se requiere una imagen como evidencia visual al cambiar el estado del ticket. " +
  ...
  if (images.length > 1) {
    throw AppError.badRequest(
      `Solo se permite adjuntar UNA imagen por cambio de estado. ` +
