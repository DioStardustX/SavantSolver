...
export class CategoriaController {
  ...
  create = async (request: Request, response: Response, next: NextFunction) => {
    try {
      ...
      if (categoriaExistente) {
        return next(AppError.badRequest("Ya existe una categoría con ese nombre"));
      }

      // Convertir slaId a número si es string, o null si no existe
      let finalSlaId: number | null = null;

      if (slaId) {
        const parsedSlaId = typeof slaId === 'string' ? parseInt(slaId) : slaId;
        if (isNaN(parsedSlaId)) {
          return next(AppError.badRequest("El ID del SLA no es válido"));
        }
      ...
      const parsedTagIds = tagIds && Array.isArray(tagIds)
        ? tagIds.map((id: any) => typeof id === 'string' ? parseInt(id) : id)
        : [];

      const parsedSpecialtyIds = specialtyIds && Array.isArray(specialtyIds)
        ? specialtyIds.map((id: any) => typeof id === 'string' ? parseInt(id) : id)
        : [];
      ...
      const nuevaCategoria = await this.prisma.category.create({
        data: {
          ...
          specialties: parsedSpecialtyIds.length > 0 ? {
            create: parsedSpecialtyIds.map((specialtyId: number) => ({
              specialtyId,
            })),
          } : undefined,
        },
  ...
  update = async (request: Request, response: Response, next: NextFunction) => {
    try {
      ...
      let finalSlaId: number | null | undefined = undefined;
      ...
      if (specialtyIds !== undefined) {
        ...
        if (Array.isArray(specialtyIds) && specialtyIds.length > 0) {
          // Convertir IDs a números
          const parsedSpecialtyIds = specialtyIds.map((specialtyId: any) =>
            typeof specialtyId === 'string' ? parseInt(specialtyId) : specialtyId
          );

          await this.prisma.categorySpecialty.createMany({
            data: parsedSpecialtyIds.map((specialtyId: number) => ({
              categoryId: id,
              specialtyId,
            })),
          });
  ...
  delete = async (request: Request, response: Response, next: NextFunction) => {
    try {
      ...
      if (ticketsAsociados > 0) {
        return next(
          AppError.badRequest(
            `No se puede eliminar la categoría porque tiene ${ticketsAsociados} ticket(s) asociado(s)`
          )
        );
      }
