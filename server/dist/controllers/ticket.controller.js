...
const ticketValidations_1 = require("../utils/ticketValidations");
...
class TicketController {
    constructor() {
        ...
        this.get = async (request, response, next) => {
            try {
                ...
                if (isNaN(userId) || isNaN(roleId)) {
                    return next(custom_error_1.AppError.badRequest("userId y roleId son requeridos y deben ser números"));
                }
                ...
                if (!role) {
                    return next(custom_error_1.AppError.notFound("Rol no encontrado"));
                }
                let whereClause = {};
        ...
        this.create = async (request, response, next) => {
            try {
                const { title, description, priority, userId, categoryId, images, } = request.body;
                if (!title || !userId || !categoryId) {
                    return next(custom_error_1.AppError.badRequest("El título, usuario y categoría son requeridos"));
                }
                ...
                if (category.sla) {
                    responseDeadline = new Date(now.getTime() + category.sla.responseTimeMinutes * 60000);
                    resolutionDeadline = new Date(now.getTime() + category.sla.resolutionTimeMinutes * 60000);
                }
        ...
        this.update = async (request, response, next) => {
            try {
                ...
                const { title, description, status, priority, assignedTechnicianId, userId, comment, justification, images, // Array de imágenes en base64
                 } = request.body;
                ...
                if (status !== undefined && status !== ticketExistente.status) {
                    ...
                    (0, ticketValidations_1.validateStatusTransition)(ticketExistente.status, status);
                    // 2. VALIDAR TÉCNICO ASIGNADO
                    // Determinar el ID del técnico que se usará (el nuevo o el actual)
                    const technicianToValidate = assignedTechnicianId !== undefined
                        ? assignedTechnicianId
                        : ticketExistente.assignedTechnicianId;
                    (0, ticketValidations_1.validateTechnicianRequired)(status, technicianToValidate);
                    // 3. VALIDAR COMENTARIO OBLIGATORIO
                    (0, ticketValidations_1.validateCommentRequired)(comment);
                    // 4. VALIDAR IMÁGENES OBLIGATORIAS
                    // EXCEPCIÓN: No se requiere imagen para asignación manual (Pendiente → Asignado)
                    const esAsignacionManual = ticketExistente.status === "Pendiente" &&
                        status === "Asignado";
                    if (!esAsignacionManual) {
                        (0, ticketValidations_1.validateImagesRequired)(images);
                    }
                    // 5. VALIDAR QUE userId ESTÉ PRESENTE
                    if (!userId) {
                        return next(custom_error_1.AppError.badRequest("Se requiere el userId para registrar el cambio de estado en el historial"));
                    }
                }
                // ============================================================
                ...
                if (status !== undefined && status !== ticketExistente.status) {
                    ...
                    if (ticketActualizado.user.id !== userId) {
                        await (0, notificationHelper_1.createTicketStatusNotification)(id, ticketActualizado.user.id, ticketExistente.status, status, ticketActualizado.title, userId);
                    }
                    // Si se asignó un técnico, notificarlo
                    if (status === "Asignado" &&
                        ticketActualizado.assignedTechnician &&
                        ticketActualizado.assignedTechnician.id !== userId) {
                        await (0, notificationHelper_1.createTicketAssignmentNotification)(id, ticketActualizado.assignedTechnician.id, ticketActualizado.title, userId);
                    }
                    // Si el ticket está en proceso o resuelto, notificar al técnico asignado
                    if ((status === "En_Proceso" || status === "Resuelto") &&
                        ticketActualizado.assignedTechnician &&
                        ticketActualizado.assignedTechnician.id !== userId) {
                        await (0, notificationHelper_1.createTicketStatusNotification)(id, ticketActualizado.assignedTechnician.id, ticketExistente.status, status, ticketActualizado.title, userId);
                    }
                }
                else {
                ...
                if (assignedTechnicianId !== undefined) {
                    ...
                    if (ticketExistente.assignedTechnicianId &&
                        ticketExistente.assignedTechnicianId !== assignedTechnicianId) {
                        await this.actualizarDisponibilidadTecnico(ticketExistente.assignedTechnicianId);
                    }
                ...
                if (status === "Cerrado" &&
                    ...
                    await this.actualizarDisponibilidadTecnico(ticketActualizado.assignedTechnicianId);
                }
        ...
        this.getAsignacionesSemana = async (request, response, next) => {
            try {
                // Obtener fecha de referencia (query param o fecha actual)
                const fechaParam = request.query.fecha;
                const tecnicoIdParam = request.query.tecnicoId;
                const fechaReferencia = fechaParam ? new Date(fechaParam) : new Date();
                if (isNaN(fechaReferencia.getTime())) {
                    return next(custom_error_1.AppError.badRequest("Fecha inválida. Usar formato YYYY-MM-DD"));
                }
                ...
                const ticketsProcesados = tickets.map((ticket) => {
                    ...
                    const porcentajeConsumido = this.calcularPorcentajeConsumido(slaTotalMinutos, slaRestanteMinutos);
                    return {
                        id: ticket.id,
                        titulo: ticket.title,
     ...
     */
    async actualizarDisponibilidadTecnico(tecnicoId) {
        ...
        console.log(`Técnico ${tecnicoId} actualizado: ${ticketsActivos} tickets activos, status: ${nuevoStatus}`);
    }
    /**
     ...
     */
    calcularUrgencia(slaRestanteMinutos) {
        if (slaRestanteMinutos < 0)
            return { nivel: "vencido", color: "#333333" };
        if (slaRestanteMinutos < 30)
            return { nivel: "critico", color: "#ff4444" };
        if (slaRestanteMinutos < 60)
            return { nivel: "alto", color: "#ffaa00" };
        if (slaRestanteMinutos < 120)
            return { nivel: "medio", color: "#ff8800" };
        return { nivel: "bajo", color: "#44ff44" };
    }
    /**
     * Calcula el porcentaje de SLA consumido
     */
    calcularPorcentajeConsumido(slaTotalMinutos, slaRestanteMinutos) {
        if (slaTotalMinutos <= 0)
            return 0;
