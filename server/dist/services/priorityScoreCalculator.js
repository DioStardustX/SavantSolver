...
exports.calculateRemainingMinutes = calculateRemainingMinutes;
exports.calculatePriorityScore = calculatePriorityScore;
exports.isTechnicianEligible = isTechnicianEligible;
exports.selectBestTechnician = selectBestTechnician;
exports.generateAssignmentJustification = generateAssignmentJustification;
/**
 ...
 */
function calculateRemainingMinutes(ticket) {
    const now = new Date();
...
function calculatePriorityScore(ticket) {
    ...
    const explanation = `Prioridad: ${ticket.priority} (${priorityValue}) × 1000 = ${priorityValue * 1000} | Tiempo restante: ${remainingMinutes} minutos | Puntaje: ${score}`;
    return {
        score,
        priorityValue,
        remainingMinutes,
 ...
 */
function isTechnicianEligible(technician, requiredSpecialtyId, workloadLimit) {
    ...
    if (requiredSpecialtyId) {
        const hasSpecialty = technician.specialties?.some((s) => s.id === requiredSpecialtyId);
        if (!hasSpecialty) {
            return {
                eligible: false,
 ...
 */
function selectBestTechnician(technicians) {
 ...
 */
function generateAssignmentJustification(ticket, technician, scoreResult, ruleName) {
    const parts = [];
    if (ruleName) {
        parts.push(`Regla aplicada: "${ruleName}"`);
    }
    parts.push(`Ticket #${ticket.id} - Prioridad: ${ticket.priority} (${scoreResult.priorityValue})`);
    parts.push(`Tiempo restante SLA: ${scoreResult.remainingMinutes} minutos`);
    parts.push(`Puntaje calculado: ${scoreResult.score}`);
    parts.push(`Técnico seleccionado: ID ${technician.id} con carga de ${technician.workload} tickets`);
